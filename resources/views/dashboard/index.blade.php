...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-sm transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-500">Total Closure</p>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-sm transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-blue-700 mt-1">{{ $totalPortActive }}</p>
            </div>
            <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-3">
            @foreach([
                ['label' => 'ODC', 'count' => $totalOdc],
                ['label' => 'ODP', 'count' => $totalOdp],
                ['label' => 'Closure', 'count' => $totalClosure],
                ['label' => 'Box Distribusi', 'count' => $totalDistributionBox],
            ] as $item)
            <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <span class="text-sm text-gray-600">{{ $item['label'] }}</span>
                <span class="px-3 py-1 bg-blue-50 text-blue-700 text-sm font-semibold rounded-lg">{{ $item['count'] }}</span>
            </div>
            @endforeach
        </div>
    </div>
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-3">
            @foreach([
                ['label' => 'Aktif', 'count' => $totalPortActive],
                ['label' => 'Kosong', 'count' => $totalPortEmpty],
                ['label' => 'Rusak', 'count' => $totalPortDamaged],
            ] as $item)
            <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <span class="text-sm text-gray-600">{{ $item['label'] }}</span>
                <span class="px-3 py-1 bg-blue-50 text-blue-700 text-sm font-semibold rounded-lg">{{ $item['count'] }}</span>
...
<div class="bg-white rounded-lg border border-gray-200 p-5 mb-6 flex items-center gap-4">
    <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
