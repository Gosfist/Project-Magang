...
<div class="max-w-2xl">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 text-sm">
        ...
        <p class="text-blue-600 text-xs mt-1">Titik: {{ $splitterOutput->splitter->networkPoint->name ?? '-' }} | Input: {{ $splitterOutput->splitter->networkInput->source_name ?? '-' }} ({{ $splitterOutput->splitter->networkInput->input_attenuation ?? '-' }} dBm)</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('splitter-outputs.update', $splitterOutput) }}" class="space-y-5">
            ...
            <div id="activeFields">
                <div class="space-y-5">
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Titik Jaringan Tujuan <span class="text-red-500" id="destRequired">*</span></label><select name="destination_network_point_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none @error('destination_network_point_id') border-red-500 @enderror"><option value="">- Tidak ada tujuan -</option>@foreach($networkPoints as $point)<option value="{{ $point->id }}" {{ old('destination_network_point_id', $splitterOutput->destination_network_point_id) == $point->id ? 'selected' : '' }}>{{ $point->name }} ({{ $point->code }})</option>@endforeach</select>@error('destination_network_point_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
