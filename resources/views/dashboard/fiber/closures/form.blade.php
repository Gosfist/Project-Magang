...
<form method="POST" action="{{ $closure->exists ? route('fiber.closures.update', $closure) : route('fiber.closures.store') }}" class="bg-white border border-gray-200 rounded-lg p-6 max-w-3xl space-y-5">
    ...
    <div>@error('nama_cl')<p class="text-red-600 text-xs mb-1">{{ $message }}</p>@enderror<label class="block text-sm font-medium mb-1">Nama CL</label><input name="nama_cl" value="{{ old('nama_cl', $closure->nama_cl) }}" required class="w-full px-4 py-2 border rounded-lg"></div>
