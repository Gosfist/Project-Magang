...
import type { MonitoringPeriod, StatItem } from './server-stats.service.js';
...
export class RouterMonitoringService {
  ...
  async getStats(nasId: number, period: MonitoringPeriod = 'today') {
    if (period === 'today') {
      return this.getToday(nasId);
    }
    if (period === 'daily') {
  ...
  private async getToday(nasId: number): Promise<{ period: string; data: StatItem[] }> {
  ...
  private async getYearly(nasId: number): Promise<{ period: string; data: StatItem[] }> {
    ...
    const currentYearMonths = await this.prisma.routerStatMonthly.findMany({
      where: { nasId, year: currentYear },
      orderBy: { month: 'asc' },
    });
    ...
    const currentYearRaw: any[] = await this.prisma.$queryRaw`
      SELECT 
        ...
        COUNT(*) as cnt
      FROM router_stats
      WHERE nas_id = ${nasId} AND recorded_at >= ${startOfCurrentYear}
    `;
    ...
    const weightedRows = currentYearMonths.map((m) => ({
      cpu: m.cpuPercent,
      mem: m.memPercent,
      down: m.netDown,
      up: m.netUp,
