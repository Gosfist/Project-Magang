...
@Component({
  ...
  templateUrl: './rasio.component.html',
})
export class RasioComponent implements OnInit, OnDestroy {
  ...
  load(): void {
    this.api
      .get<{ data: MainCoreNode[]; meta: PageMeta }>(
        `/main-core/rasio?search=${encodeURIComponent(this.search)}&page=${this.page()}`
      )
      .subscribe({
        next: (response) => {
  ...
  fetchParents(): void {
    ...
    const query = `/main-core/rasio/parents?search=${encodeURIComponent(this.parentSearch)}${
  ...
  onPortSelect(): void {
    this.form.redamanIn = this.autoRedaman();
  }
  ...
  autoRedaman(): string {
    ...
    if (parent.type === 'rasio' && this.form.parentPortOut) {
      const percentage = Number(
        (parent.rasioRedamanPorts[this.form.parentPortOut] ?? '').replace('%', '').replace(',', '.')
      );
  ...
  save(): void {
    ...
    const body = {
      ...
      spesifikasi: { rasio_redaman_ports: { '1': this.form.ratio1, '2': this.form.ratio2 } },
    };
    const request = editing
      ? this.api.patch<{ message: string }>(`/main-core/rasio/${editing.id}`, body)
      : this.api.post<{ message: string }>('/main-core/rasio', body);
    request.subscribe({
      next: (response) => {
        this.open.set(false);
  ...
  remove(node: MainCoreNode): void {
    ...
    this.api.delete<{ message: string }>(`/main-core/rasio/${node.id}`).subscribe({
      next: (response) => {
        this.toast.set({ message: response.message, type: 'success' });
        this.load();
      },
