...
export class OdpComponent implements OnInit, OnDestroy {
  ...
  load(): void {
    this.api.get<{ data: MainCoreNode[]; meta: PageMeta }>(`/main-core/odp?search=${encodeURIComponent(this.search)}&page=${this.page()}`).subscribe({
  ...
  fetchParents(): void {
    ...
    const query = `/main-core/odp/parents?search=${encodeURIComponent(this.parentSearch)}${
  ...
  autoRedaman(): string {
    ...
    const connectorLoss = parent.type === 'odc' ? 1 : 0;
    return ((parent.redamanIn ?? 0) - splitterLoss - (distance / 1000) * 0.35 - connectorLoss - 1).toFixed(2);
  }
  ...
  save(): void {
    ...
    const request = editing ? this.api.patch<{ message: string }>(`/main-core/odp/${editing.id}`, body) : this.api.post<{ message: string }>('/main-core/odp', body);
  ...
  remove(node: MainCoreNode): void {
    ...
    this.api.delete<{ message: string }>(`/main-core/odp/${node.id}`).subscribe({ next: (response) => { this.toast.set({ message: response.message, type: 'success' }); this.load(); }, error: (error) => this.toast.set({ message: error.message, type: 'error' }) });
  }
}
