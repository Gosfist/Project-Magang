...
export class PsbComponent implements OnInit {
  ...
  formOpen = signal(false); formStep = signal(1); activationOpen = signal(false); activationStep = signal(1); editing = signal<PsbOrder | null>(null); selected = signal<PsbOrder | null>(null);
  ...
  nextFormStep() { if (!this.form.customerName.trim() || !this.form.phone.trim() || !this.form.idCardNumber.trim() || !this.form.idCardPhoto || !this.form.address.trim() || !this.form.latitude || !this.form.longitude) return this.toast.set({ message: 'Lengkapi seluruh data pelanggan terlebih dahulu.', type: 'error' }); this.formStep.set(2); }
  ...
  openActivation(item: PsbOrder) { this.selected.set(item); this.activationStep.set(1); this.activation = { username: item.customerId, password: item.customerId, odp: '', routerNasId: '', installationPhoto: '' }; this.activationOpen.set(true); }
