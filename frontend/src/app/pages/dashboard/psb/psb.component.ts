...
export class PsbComponent implements OnInit {
  ...
  form = { customerName: '', phone: '', address: '', pppoePackageId: '', areaId: '' };
  ...
  openForm(item?: PsbOrder) { this.editing.set(item || null); this.form = item ? { customerName: item.customerName, phone: item.phone, address: item.address, pppoePackageId: item.pppoePackageId, areaId: item.areaId || '' } : { customerName: '', phone: '', address: '', pppoePackageId: '', areaId: '' }; this.formOpen.set(true); }
