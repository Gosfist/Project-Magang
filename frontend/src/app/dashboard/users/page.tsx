...
export default function UsersPage() {
    ...
    const load = useCallback(
        () =>
            api<{ data: User[]; meta: PageMeta }>(
                `/users?search=${encodeURIComponent(search)}&role=${role}&status=${status}&page=${page}`,
            )
                .then((r) => {
                    setItems(r.data);
                    setMeta(r.meta);
    ...
    async function remove(user: User) {
        ...
        try {
            const r = await api<{ message: string }>(`/users/${user.id}`, {
                method: "DELETE",
            });
            setToast({ message: r.message, type: "success" });
            load();
    ...
    return (
        <div className="space-y-4">
                    ...
                    <tbody>
                        {items.map((user) => (
                            <tr key={user.id}>
                                ...
                                <td>
                                    <span
                                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${user.status === "active" ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-600"}`}
                                    >
                                        {user.status}
                                    </span>
                                </td>
            ...
            <Modal
                open={open}
                title={editing ? "Edit Data Petugas" : "Tambah Petugas"}
